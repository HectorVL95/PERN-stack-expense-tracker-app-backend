...
const userQuieres_1 = require("./userQuieres");
...
exports.createUser = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    const newUser = await db_1.default.query(userQuieres_1.createUserQuery, [name, email, hashedPassword, budget]);
    res.status(201).json({
        success: true,
...
exports.loginUser = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    const user = await db_1.default.query(userQuieres_1.loginUserQuery, [email, password]);
    if (!user)
        return next(new errorResponse_1.errorResponse('User not found', 401));
    const token = jsonwebtoken_1.default.sign({ userId: user.oid, email: email }, process.env.JWT_SECRET, { expiresIn: '999999h', algorithm: 'HS256' });
...
exports.getLoggedUser = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    const user = await db_1.default.query(userQuieres_1.findUserByIdQuery, [userId]);
...
exports.deleteUser = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
    ...
    const deleteUser = await db_1.default.query(userQuieres_1.deleteUserQuery, [userId]);
    if (!deleteUser)
        return next(new errorResponse_1.errorResponse('User does not exist', 400));
    res.status(200).json({
        success: true,
...
exports.modifyUser = (0, asyncHandler_1.asyncHandler)(async (req, res, next) => {
    const { userId } = req.body;
    const { name } = req.body;
    const result = await db_1.default.query(userQuieres_1.modifiUsersQuery, [name, userId]);
